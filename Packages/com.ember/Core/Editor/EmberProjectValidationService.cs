...
namespace Ember.Core.Editor
{
    ...
    internal static class EmberProjectValidationService
    {
        ...
        internal static EmberProjectValidationReport Scan()
        {
            ...
            var editing = EmberProjectSetup.IsEmbeddedPackage() ? EmberProjectSetup.GetEditingTemplate() : null;
            ...
            if (template == null)
            {
                ...
                report.Add(EmberValidationSeverity.Warning, "比较基准", report.Baseline,
                    suggestion: "在项目初始化页确认活动模板，或在模板开发页加载目标模板。");
            }
            else
            {
                report.Baseline = DescribeBaseline(template, editing, active);
                RunCheck(report, "模板存储", () => ValidateTemplate(template, report));
        ...
        private static void ValidateTemplate(TemplateInfo template, EmberProjectValidationReport report)
        {
