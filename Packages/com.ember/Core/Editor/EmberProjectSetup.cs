...
namespace Ember.Core.Editor
{
    ...
    public static class EmberProjectSetup
    {
        ...
        public static TemplateSwitchResult SwitchTemplate(string templateId)
        {
            ...
            var sourceAssets = Path.Combine(packagePath, "Templates~", templateId, "Assets");
        ...
        /// <summary>按部署器规则在内存中生成模板文件的消费端形态；不写模板或项目。</summary>
        internal static byte[] ReadDeploymentTemplateBytes(
            string filePath,
            string version,
            string frameworkVersion)
        {
            return RewriteVersionMarker(
        ...
        private static bool RewriteVersionMarker(string fullPath, string version, string frameworkVersion)
        {
            ...
            var replaced = RewriteVersionMarker(
                bytes,
                version,
                frameworkVersion,
                out var changed);
            ...
            try { File.WriteAllBytes(fullPath, replaced); }
            catch { return false; /* 写失败不影响部署，标记保持原样 */ }
            return true;
        }

        /// <summary>纯内存版本头改写，供部署和只读校验共用同一规则。</summary>
        private static byte[] RewriteVersionMarker(
            byte[] bytes,
            string version,
            string frameworkVersion,
            out bool changed)
        {
            changed = false;
            if (bytes == null || string.IsNullOrEmpty(version)) return bytes;

            bool hasBom = bytes.Length >= 3 && bytes[0] == 0xEF && bytes[1] == 0xBB && bytes[2] == 0xBF;
            var enc = new System.Text.UTF8Encoding(hasBom);
            string text = enc.GetString(bytes);
            if (!text.Contains("Generated by Ember Setup")) return bytes;
            ...
            string requestedTemplateId)
        {
            ...
            data.records ??= new List<DeployedTemplateRecord>();
            if (!string.IsNullOrEmpty(data.activeTemplateId)
                && data.records.All(record => !string.Equals(
                    record.templateId,
                    data.activeTemplateId,
                    StringComparison.Ordinal)))
            {
            ...
            if (data.records.Count == 0)
                return $"项目尚未部署模板，请直接初始化 [{requestedTemplateId}]。";
        ...
        private static void RegisterBuildSettings(bool pruneMissingScenes = false)
        {
            ...
            if (pruneMissingScenes)
                scenes.RemoveAll(scene => string.IsNullOrEmpty(scene.path)
                    || !File.Exists(ToFullPath(scene.path)));

            // FrameworkScene 固定 index 0
            scenes.RemoveAll(scene => scene.path == FrameworkScenePath);
            if (File.Exists(ToFullPath(FrameworkScenePath)))
