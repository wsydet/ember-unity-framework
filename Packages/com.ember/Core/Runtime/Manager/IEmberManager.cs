namespace Ember.Core
{
    ...
    ///
    /// <b>定位：框架必要管理器，不是可选业务模块。</b>
    /// 实现此接口的类代表“无论什么游戏状态都需要在线”的基础服务
    /// （事件总线、资源加载、Update 驱动等）。它们在应用启动时由
    /// <see cref="EmberManagerCollector.InitializeAll"/> 统一初始化，
    /// 应用退出时逆序销毁。
    /// Manager 构成所有游戏共享的稳定底座，不按业务 Phase 启停，也不依赖具体
    /// <see cref="IEmberModule"/>；不同游戏在这套底座上组合不同 Module。
    ///
    /// <b>业务模块请用 <see cref="IEmberModule"/>。</b>
    /// 业务模块的生命周期由 <see cref="EmberModuleCollector"/> 按 Phase 驱动。
