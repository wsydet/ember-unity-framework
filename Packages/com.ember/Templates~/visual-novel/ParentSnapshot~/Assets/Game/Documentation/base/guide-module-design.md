...
> 文档归属：base 基础模板；派生模板继承。
> 整理日期：2026-09-17；保留原设计、扩展待办及历史验证边界。
...
1. **创建引导定义**：`Ember → 引导 → 引导编辑器 → 新建引导`，选中步骤编辑条件 / 事件 / 执行器；也可双击已有 GuideDefine。详见 [引导编辑器使用](guide-editor-usage.md)。
...
2026-09-12 用户已通过模板面板将本轮编辑器和原生 EntityId 回调保存并封存为 base 0.6.3，再同步封存 source3d-2p5d 0.3.5；父版本、实际内容与封存 Hash 一致，随框架 0.12.7 交付。两个 GuideModule 均保持 Enabled=false。此处版本为历史交付记录；当前编辑模板以项目中心显示为准。

开发工程回归位于 Assets/Tests/Editor/GuideEditor，3 个 EditMode 用例覆盖嵌套引用深复制、新建空步骤与 Undo、校验不修改资产。该测试目录不属于业务模板快照。该批次记录未完成 Unity 编译验证及测试执行，不作为当前版本通过的证据。消费项目不随模板取得 Assets/Tests，开发仓库可运行对应测试；实际使用仍需验证编辑和运行行为。
