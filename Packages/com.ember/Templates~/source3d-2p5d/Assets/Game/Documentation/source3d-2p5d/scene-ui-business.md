...
> 归属：source3d-2p5d 模板的业务示例。通用 SceneUI 引擎与 ModuleBase 文档位于 Ember 包内，不属于本模板专属内容。
...
模块装配由 `EmberModuleAttribute.Enabled` 控制；当前 `[EmberModule(ModulePhase.Gameplay)]` 使用默认启用值。Collector 在 Init 阶段发现并构造模块，进入 Gameplay Phase 才初始化；设为 `Enabled = false` 后不会装配。
