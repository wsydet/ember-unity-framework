...
当前已发布版本为 **0.12.1**；开发基线使用 Unity `6000.5.4f1`：

```json
{
  "com.ember": "https://github.com/wsydet/ember-unity-framework.git?path=/Packages/com.ember#v0.12.1"
}
```
...
完整工程的 56 项直接依赖见随包 [消费端依赖声明](Dependencies~/README.md) 和 [0.12.1 manifest 基线](Dependencies~/manifest-0.12.1.json)。Table 可视化与单表导出不增加第三方依赖，继续复用 `ember-v0.11.1`；消费端按包名合并清单，保留自己的其他依赖，不能覆盖整份项目 manifest。
