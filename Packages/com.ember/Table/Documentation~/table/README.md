...
```csharp
if (GameTableModule.Instance.IsReady
    && GameTableModule.Instance.Database.TryGetTable<MyRow>("my_table", out var table)
...
字符串键和 Table ID 使用 Ordinal、大小写敏感。枚举保持源行顺序。默认只有主键 Dictionary；高频非主键查询必须在 Definition 显式声明，由 Binding 构建 `EmberTableIndex<TKey,TRow>`。
...
4. “烘焙并生成全部”会先暂存全部二进制和 C#，逐表用 Runtime Codec 回读比对，再按所有权清单整批提交；任一步失败零写入或完整回滚。
5. 生成 `<RowType>.TableBinding.g.cs` 与 `GameTables.Catalog.g.cs`。数据值变化不会改变 C#；Schema、路径或表清单改变时才更新生成代码。

“校验当前”只报告所选 Definition（引用目标仍参与校验）；“烘焙当前（整批）”为了保持 Catalog 与所有权清单一致，仍按全量事务提交。工具可定位 Definition、Row、源文件和二进制，并通过正式 Runtime Binding 显示元信息及最多 200 行只读数据。
