...
- **设置界面语言选项**：设置面板新增语言行（简体中文 / 繁體中文 / 日本語 / English），选择写入 PlayerPrefs、不进存档、不影响剧情指纹；默认中文走源语言列回退。设置面板由可重入的模板升级自动补行并重新生成绑定。
- **编辑期预览**：新增编辑期多语言解析器装配，`TMPEx` Inspector 的多语言区可逐语言预览；编辑期不改写文本。
- **测试与工程卫生**：Play Mode 用例统一场景脚手架（不再留下未保存场景、退出 Play 只在真的处于 Play Mode 时执行）；测试运行前后自动还原被 Unity 改写的非业务文件；检查点版本断言改用 `NovelCheckpoint.CurrentSchemaVersion`；外观用例断言落在可见图片上。框架 API、其他模板及第三方依赖不变。
## [0.14.10] - 2026-09-24

- **视觉小说模板 0.9.0**：新增整数赋值、加减乘除取模、变量操作数与范围随机数；通过随机区间和条件分支配置概率、权重事件及次数触发逻辑。
- **存档与文字**：随机状态随 schema 7 检查点保存，读档不重复扣费或重掷；动态文字显式绑定变量，保留剧情源文本；校验变量声明、除零、溢出和绑定冲突。
- **剧情导入技能**：支持整本飞书表格逻辑审计、完整性检查和概率/数值映射；策划未定规则交由项目配置，本次未导入源剧情。
- **验证与迁移**：Unity MCP 编译、70 项相关回归及最终 19 项专项通过（两轮部分重叠）。模板谱系与技能同步通过；未重跑全套、Play Mode 或消费项目迁移。0.8.x → 0.9.0 跨 minor，先备份定制内容再迁移。框架 API、其他模板及第三方依赖不变。

## [0.14.9] - 2026-09-24

- **视觉小说模板 0.8.0**：主 UI 编辑器开放全屏旁白、章节卡、点击区域、字号及历史弹窗的布局与外观；章节卡点击后整体渐隐，文字支持渐显、打字机或立即显示，时长、速度和缓动可配置。
- **二级步骤与参考剧情**：新增彩色可折叠组合、隐藏所有立绘、舞台复位、场景收尾及可保存/复用的自定义预设。复制会重建内部动作和等待引用；LastLight 提供 15 组示例、3 份预设，保留 80 条台词及分支。
- **技能与清理**：剧情导入、演出优化、剧情导出三个技能支持二级步骤和文字过渡，最低业务模板版本 0.8.0；运行入口只保留 LastLight，旧示例移为测试夹具。
- **兼容修复**：修复旧剧情新增文字参数反序列化为零造成的 BadText 与加载失败；模块销毁通过显式接口接入。
- **验证与迁移**：177 项相关回归通过，包含实际页面、Play Mode 及正式 Prefab 布局/透明度；未重跑完整 645 项或验收消费项目。0.7.x → 0.8.0 跨 minor，先保护定制内容再按项目中心迁移；仅升级框架不自动更新业务 Assets。框架 API、其他模板及第三方依赖不变。
