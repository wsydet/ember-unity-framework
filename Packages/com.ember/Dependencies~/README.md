...
0.12.9 修复快速打开场景在刷新后依赖 SO Inspector 才能打开的问题，直接按绑定场景的真实路径解析，保留刷新选择并检查无效场景。Unity MCP 不可用，编译、4 个新增 EditMode 案例及消费端交互尚未验证。

当前发布基线是 `0.12.9`；`manifest-0.12.9.json` 与 `release-0.12.9.json` 是正式发布声明。模板保持 base 0.6.3 与 source3d-2p5d 0.3.5。

## 这份清单的范围

- `manifest-0.12.9.json` 是完整开发环境的可移植依赖基线：当前项目 manifest 的 55 项直接依赖，加上 Feel，共 56 项。
- 框架固定为 `v0.12.9`；本次未改变第三方包，7 个第三方包继续固定到私有仓库已发布的 `ember-v0.11.1`；Unity MCP 固定到开发工程 lock 中实际解析的 commit，不使用浮动 `main`。
...
- `release-0.12.9.json` 记录第三方包版本、模板兼容声明和可按需省略的开发工具。完整清单包含开发工具，不意味着每个消费工程都必须使用全部工具。
- 0.12.9 不增加第三方依赖；本版清单只把 `com.ember` 目标推进到 `v0.12.9`，其余来源与版本保持一致。
...
升级后只读核对实际包与锁定提交，再手动编译并验证“刷新 → 选择主场景/叠加场景 → 打开场景”，无需先切换到 SO 面板。无需重新部署模板。
正式规则见 [升级规则 §7.1](../Documentation~/maintenance/unityfarm-change-routing.md#71-仅框架变化)。

## 首次安装与依赖配置

1. 先确认第三方仓库 `ember-v0.11.1` 和框架仓库 `v0.12.9` 已真正发布，且消费机器有私有仓库访问权限及适用的插件授权。
...
第三方内容未变，复用已发布的 `ember-v0.11.1`；框架发布 `v0.12.9`。已有项目升级包后不会覆盖业务模板文件。

0.12.9 按 `package.json → CHANGELOG → release/manifest → commit → tag → push` 发布。此次发布快速场景打开器修复、编辑器场景路径属性及相关回归；模板、共享字体和第三方依赖不变。
