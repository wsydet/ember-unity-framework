...
0.12.10 在 UPM 管理器中展示每个可升级版本的更新内容，默认展开最新版本，异步读取固定标签的发布日志。已验证远端中文日志读取；Unity MCP 不可用，编译、5 个新增 EditMode 案例及消费端交互尚未验证。

当前发布基线是 `0.12.10`；`manifest-0.12.10.json` 与 `release-0.12.10.json` 是正式发布声明。模板保持 base 0.6.3 与 source3d-2p5d 0.3.5。

## 这份清单的范围

- `manifest-0.12.10.json` 是完整开发环境的可移植依赖基线：当前项目 manifest 的 55 项直接依赖，加上 Feel，共 56 项。
- 框架固定为 `v0.12.10`；本次未改变第三方包，7 个第三方包继续固定到私有仓库已发布的 `ember-v0.11.1`；Unity MCP 固定到开发工程 lock 中实际解析的 commit，不使用浮动 `main`。
...
- `release-0.12.10.json` 记录第三方包版本、模板兼容声明和可按需省略的开发工具。完整清单包含开发工具，不意味着每个消费工程都必须使用全部工具。
- 0.12.10 不增加第三方依赖；本版清单只把 `com.ember` 目标推进到 `v0.12.10`，其余来源与版本保持一致。
...
升级后只读核对实际包与锁定提交，再手动编译；安装 0.12.10 后，检测到后续可升级版本时可展开更新内容。旧版管理器不会在升级前获得这项能力。无需重新部署模板。
正式规则见 [升级规则 §7.1](../Documentation~/maintenance/unityfarm-change-routing.md#71-仅框架变化)。

## 首次安装与依赖配置

1. 先确认第三方仓库 `ember-v0.11.1` 和框架仓库 `v0.12.10` 已真正发布，且消费机器有私有仓库访问权限及适用的插件授权。
...
第三方内容未变，复用已发布的 `ember-v0.11.1`；框架发布 `v0.12.10`。已有项目升级包后不会覆盖业务模板文件。

0.12.10 按 `package.json → CHANGELOG → release/manifest → commit → tag → push` 发布。此次发布 UPM 版本更新内容、异步读取与失败重试及相关回归；模板、共享字体和第三方依赖不变。
