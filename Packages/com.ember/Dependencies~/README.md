...
当前已发布基线是 `0.12.1`；`manifest-0.12.1.json` 与 `release-0.12.1.json` 是配置表可视化和单表导出补丁的正式发布声明。模板内容未变化，仍使用已封存的两份 0.12.0 兼容模板。

## 这份清单的范围

- `manifest-0.12.1.json` 是完整开发环境的可移植依赖基线：当前项目 manifest 的 55 项直接依赖，加上 Feel，共 56 项。
- 框架固定为 `v0.12.1`；本次未改变第三方包，7 个第三方包继续固定到私有仓库已发布的 `ember-v0.11.1`；Unity MCP 固定到开发工程 lock 中实际解析的 commit，不使用浮动 `main`。
...
- `release-0.12.1.json` 记录第三方包版本、模板兼容声明和可按需省略的开发工具。完整清单包含开发工具，不意味着每个消费工程都必须使用全部工具。
- 0.12.1 不增加第三方依赖；本版清单只把 `com.ember` 目标推进到 `v0.12.1`，其余来源与版本保持一致。

## 如何使用

1. 先确认第三方仓库 `ember-v0.11.1` 和框架仓库 `v0.12.1` 已真正发布，且消费机器有私有仓库访问权限及适用的插件授权。
...
第三方内容未变，复用已发布的 `ember-v0.11.1`；框架发布 `v0.12.1`。Table 模板结构沿用 0.12.0，已有项目升级包后不会覆盖业务模板文件。

0.12.1 按 `package.json → CHANGELOG → release/manifest → commit → tag → push` 发布。此次只升级框架 Editor 工具和事务代码；模板内容、版本、Hash 与父快照均不变。
