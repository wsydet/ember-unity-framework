# Ember 0.11.3 消费端依赖声明

## 这份清单的范围

- `manifest-0.11.3.json` 是完整开发环境的可移植依赖基线：当前项目 manifest 的 55 项直接依赖，加上 Feel，共 56 项。
- 框架固定为 `v0.11.3`；本次未改变第三方包，7 个第三方包继续固定到私有仓库已发布的 `ember-v0.11.1`；Unity MCP 固定到开发工程 lock 中实际解析的 commit，不使用浮动 `main`。
...
- `release-0.11.3.json` 记录第三方包版本、模板兼容声明和可按需省略的开发工具。完整清单包含开发工具，不意味着每个消费工程都必须使用全部工具。

## 如何使用

1. 先确认第三方仓库 `ember-v0.11.1` 和框架仓库 `v0.11.3` 已真正发布，且消费机器有私有仓库访问权限及适用的插件授权。
...
0.11.3 保持 `base 0.5.5` 与 `source3d-2p5d 0.2.6` 的模板内容不变，并沿用 0.11.2 的跨 Git 行尾稳定 hash。消费端只部署目标完整模板，不能保存、加载或修改包内模板；部署另一模板时以事务替换目标模板管理的业务目录。

## 发布顺序

第三方内容未变，复用已发布的 `ember-v0.11.1`；框架发布 `v0.11.3`。消费项目更新后仍需完成目标模板部署、项目校验和运行回归。
