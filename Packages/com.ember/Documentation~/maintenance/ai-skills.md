...
0.14.7 新增第 10 项框架级技能 `ember-template-conflict-recovery`，最低框架版本为 **0.14.5**。它读取项目中心的模板增量冲突日志，比较旧模板、新模板与本地内容，保留业务改动并交回项目中心重新预览；完整部署后恢复需要可信的部署前备份。它不会直接修改旧基线、包内模板或部署版本。

消费项目选择技能来源标签 v0.14.7 后，可在 `Ember/UPM Manager → AI Skill · 独立安装与更新` 检查并安装 **Ember 模板冲突恢复**，重新加载会话后调用：

```text
$ember-template-conflict-recovery 合并这份模板增量冲突日志对应的本地文件，保留业务改动，并给出重新预览后的逐项选择建议。
```

固定旧标签不会包含新技能；仅在框架开发目录新增文件也不会自动安装到其他项目。随包首次安装还需在技能源提交后运行 `scripts/build-ai-skill-bundle.ps1` 并通过 `-Check`，再随框架发布；不能手工修改生成 bundle 或用未提交源伪造 sourceCommit。0.14.7 包内 bundle 已包含全部 10 项框架技能；已有项目仍需显式检查并安装新技能。
...
- **发布目录**：`.agents/skills/catalog.json`。只有列入目录的技能可通过面板安装。v0.13.1 含 9 项，v0.14.7 清单及包内 bundle 含 10 项，新增项见上文；插件迁移与空白模板不在目录内。
