...
- **仅更新模板技能（0.14.4 起）**：在项目中心“模板专属 AI Skill”预览并显式更新；只同步专用技能源、发现副本与技能记录，保留业务内容和业务部署基线。兼容下限按实际业务版本检查；不要求完整重新部署。详见 [模板技能](template-ai-skills.md)。

- **补丁增量更新（0.14.5 起）**：同模板 major.minor 一致且 patch 递增时，基于旧部署原稿、新版模板和本地文件三方比较；保留本地独有内容与单方修改，冲突逐项选择后再事务提交。项目中心可复制冲突日志，交给 `$ember-template-conflict-recovery`（Ember 模板冲突恢复）协助合并，随后重新预览。技能需先通过 UPM Manager 的 AI Skill 更新入口安装；安装与发布状态见 [AI Skill 独立更新](ai-skills.md)。
- **补齐缺失**：只允许相同版本/hash 修复缺失文件，不推进版本与基线，不作为升级入口。
- **完整重新部署**：事务替换五个模板管理目录，会覆盖其中的 UnityFarm 修改；只可在备份和确认差异后使用。
- **人工迁移**：用于增量冲突的业务合并、历史基线无法核实或需要跨版本适配的情况。对照新模板移植必要变化，并保留产品代码。

模板前两位变化时，先保护本地改动，再完整部署并恢复；patch 必须保持业务扩展点、资源 GUID 和序列化兼容，不能用 patch 发布需要重建项目的结构变化。
部署原稿保存于 `ProjectSettings/EmberTemplateBaseline~/`，应与部署记录一起提交。旧项目可导入与部署 hash 匹配的历史模板 Assets 恢复基线，无需重新部署；缺失或错误的历史记录不能用当前业务内容猜测。
当前增量支持文件级选择，尚无自动用户代码区/场景合并；0.14.5 已通过 155 项相关 EditMode 测试，真实消费项目端到端升级仍需验收。
...
6. UnityFarm 必须通过 `Ember/UPM Manager` 升级框架，禁止直接修改 manifest 或 lock 来升级；如模板有变化，再按 7.2 节选择补丁增量、完整重新部署或人工迁移。
