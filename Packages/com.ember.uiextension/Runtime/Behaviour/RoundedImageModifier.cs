...
//    {
//        const int MaxTriangleNum = 16;
//        const int MinTriangleNum = 4;
//        
//        protected Image TargetImage => graphic as Image;
//
//        [Header("圆角半径")]
...
//        {
//            var padding = TargetImage.overrideSprite == null ? Vector4.zero : DataUtility.GetPadding(TargetImage.overrideSprite);
//            Rect r = TargetImage.GetPixelAdjustedRect();
//            var size = TargetImage.overrideSprite == null ? new Vector2(r.width, r.height) : new Vector2(TargetImage.overrideSprite.rect.width, TargetImage.overrideSprite.rect.height);
...
//            Vector4 uv = TargetImage.overrideSprite != null ? DataUtility.GetOuterUV(TargetImage.overrideSprite) : Vector4.zero;
//
//            vh.Clear();
...
//            //计算出uv中对应的半径值坐标轴的半径
//            float uvRadiusX = radius / (v.z - v.x) * (uv.z - uv.x);
//            float uvRadiusY = radius / (v.w - v.y) * (uv.w - uv.y);
//
//            //0，1
...
//
//            //开始构造四个角
//            List<Vector2> vCenterList = ListPool<Vector2>.Get();
//            List<Vector2> uvCenterList = ListPool<Vector2>.Get();
//            List<int> vCenterVertList = ListPool<int>.Get();
//
//            //右上角的圆心
