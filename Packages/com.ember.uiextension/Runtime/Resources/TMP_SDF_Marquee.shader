Shader "TextMeshPro/Distance Field(Marquee)" {
...
Properties {
	...
	[HideInInspector]_Rectangle("Rectangle",vector) = (0,0,100,0)
...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			vert.x = vert.x - (_Time.y - _StartTime) * _Speed;
			float width = _Rectangle.z - _Rectangle.x + 20;
			//input.color = (vert.x - _Rectangle.x) / width;
			vert.x = frac((vert.x - _Rectangle.x - 20) / width) * width + _Rectangle.x;
