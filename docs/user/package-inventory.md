...
最后核对：2026-09-07；按当前 `manifest.json`、`packages-lock.json` 和 embedded `package.json` 统计，不代表远程最新版本。
...
| com.ember | 0.11.1（声明，未发布） | Ember 框架（单包合一）：事件/资源/UI/场景/音频/相机/输入 + 状态机 + 编辑器工具 + UI 绑定代码生成；**内置 UniTask（Unity 6000.5 TreeView 泛型修复版，MIT 随包分发）** | 框架自带 → embedded |
...
已新增随框架分发的 [0.11.1 消费端依赖声明](../../Packages/com.ember/Dependencies~/README.md)：当前 55 项直接依赖加 Feel，共 56 项；7 个私有第三方包固定到待发布的 `ember-v0.11.1`，MCP 固定到当前 lock 的 commit。该清单独立于本工程 manifest，不改变这里记录的实际安装来源。
