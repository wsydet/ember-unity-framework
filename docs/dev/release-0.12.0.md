# Ember Framework 0.12.0 候选发布说明

状态：候选实现、模板内容、框架兼容声明和静态校验已收口，**尚未发布、尚未创建 tag**。新消费项目验收仍待完成。
...
- 框架候选版本：`0.12.0`；建议发布 tag：`v0.12.0`。
...
- [ ] 新消费项目使用最终发布提交的 Package 内容，部署模板、启用 Module、启动并读取测试表通过。
- [x] 用户已明确授权完成收口后提交、创建不可变 `v0.12.0` tag 并推送。

只有全部 Gate 完成后，才能把本文件改为正式发布日期并发布 tag。
