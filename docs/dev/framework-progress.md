...
> 核对日期：2026-09-07。实现以当前工作区为准；包版本与本地 Git tag 分别记录，远程发布状态不由本次维护推断。
...
| 模板与编辑器 | 项目中心、schema v2、父子三方同步、场景语义合并、事务回滚、活动模板保护 | [模板体系](template-upgrade-system.md)、[包维护](upm-migration-plan.md) |
...
| P1 | 新消费项目与发布验收 | 重新记录当前工作区编译/测试结果；验证首次部署派生模板、跨模板阻止、场景引用和 Play 链路；不在文档维护中执行发布 |
