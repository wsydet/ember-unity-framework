...
| 框架包 | 已发布 `0.12.2` | Table 生成脚本及接线工具编码修复 |
| 发布状态 | `v0.12.2` | 用户确认框架项目测试通过；UnityFarm 消费回归待完成 |
...
随包 `Dependencies~` 已提供 56 项直接依赖的可移植 manifest 和 0.12.2 正式发布声明；本补丁不增加第三方依赖，7 个第三方包继续固定到已发布的 `ember-v0.11.1`。消费端仍需手动合并配置，详见 [0.12.2 发布说明](release-0.12.2.md)。
...
| 优先级 | 事项 | 当前边界 |
|---|---|---|
| P0 | UnityFarm 0.12.2 消费验收 | 通过 UPM Manager 更新至不可变 `v0.12.2`，验证配置表窗口、单表/全量导出、Module 启动与实际读表 |
