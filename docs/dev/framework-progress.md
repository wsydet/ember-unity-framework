...
0.12.10 为 UPM 管理器增加可升级版本的更新内容，默认展开最新版本；通过现有 Git 凭据异步读取固定标签的 CHANGELOG，支持缓存、超时、重试和原文入口。远端中文日志读取已验证；Unity 编译、5 个新增 EditMode 案例与消费交互待验证。见 [0.12.10 发布说明](release-0.12.10.md)。

0.12.9 修复快速打开场景在刷新后依赖 SO Inspector 更新名称缓存的问题。按绑定资产的路径解析场景，刷新保留选择并在切换前校验全部所选场景。新增 4 个 EditMode 回归；Unity MCP 不可用，编译、测试与消费端交互未验证。模板、字体和第三方依赖不变。见 [0.12.9 发布说明](release-0.12.9.md)。
...
| 框架包 | 已发布 `0.12.11` | 可选包安装状态与固定版本安装 |
| 发布状态 | `v0.12.11` | 安装标签与静态检查已核对；Unity 编译、8 个新增 EditMode 案例与实际安装待确认 |
...
随包 `Dependencies~` 已提供 56 项直接依赖的可移植 manifest 和 0.12.11 正式发布声明；完整 manifest 的第三方基线仍为 `ember-v0.11.1`。可选安装按钮改用各包独立标签（Feel 除外），对应地址单独记入发布声明。消费端框架升级必须通过 Ember/UPM Manager；第三方依赖按实际需要点击安装，详见 [0.12.11 发布说明](release-0.12.11.md)。
...
| P0 | 快速场景打开器 0.12.9 验收 | 手动编译并验证刷新后直接打开、主场景与叠加场景选择；执行新增 4 个 EditMode 案例 |
