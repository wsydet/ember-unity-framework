...
> 核对日期：2026-09-12。实现以当前工作区与发布 tag 为准；消费项目验收单独记录。
...
| 框架包 | 已发布 `0.12.6` | EUI 中文用途与基础 UI 删除保护 |
| 发布状态 | `v0.12.6` | 静态检查完成；Unity 编译、EditMode 与交互回归待确认 |
| 根模板 | `base 0.6.1 / stable` | 内容与封存 Hash 均为 `b293c611af0ae8b4b78000977e071652`；已通过项目中心声明框架 `0.12.0` |
| 派生模板 | `source3d-2p5d 0.3.3 / preview` | 父基线 `base 0.6.1`，ParentSnapshot 与父 Hash 一致；已通过父同步继承框架 `0.12.0` |
| 模板开发副本 | `source3d-2p5d 0.3.3` | `Assets/Editor/EmberEditingTemplate.json` 记录 Hash 与当前模板一致，不代表消费项目验收通过 |
...
随包 `Dependencies~` 已提供 56 项直接依赖的可移植 manifest 和 0.12.6 正式发布声明；本补丁不增加第三方依赖，7 个第三方包继续固定到已发布的 `ember-v0.11.1`。消费端框架升级必须通过 Ember/UPM Manager；第三方依赖按实际需要配置，详见 [0.12.6 发布说明](release-0.12.6.md)。
