...
| 框架包 | 已发布 `0.12.9` | 快速场景打开器刷新修复 |
| 发布状态 | `v0.12.9` | 静态检查完成；Unity 编译、4 个新增 EditMode 案例与消费交互待确认 |
...
随包 `Dependencies~` 已提供 56 项直接依赖的可移植 manifest 和 0.12.9 正式发布声明；本补丁不增加第三方依赖，7 个第三方包继续固定到已发布的 `ember-v0.11.1`。消费端框架升级必须通过 Ember/UPM Manager；第三方依赖按实际需要配置，详见 [0.12.9 发布说明](release-0.12.9.md)。
