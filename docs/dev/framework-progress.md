...
> 核对日期：2026-09-10。实现以当前工作区与发布 tag 为准；消费项目验收单独记录。
...
| 框架包 | 已发布 `0.12.3` | 共享字体原始二进制恢复与完整性回归 |
| 发布状态 | `v0.12.3` | 静态完整性检查通过；Unity 编译、UPM 实际解析与中文标记运行验收单独待确认 |
...
随包 `Dependencies~` 已提供 56 项直接依赖的可移植 manifest 和 0.12.3 正式发布声明；本补丁不增加第三方依赖，7 个第三方包继续固定到已发布的 `ember-v0.11.1`。消费端仍需手动合并配置，详见 [0.12.2 发布说明](release-0.12.2.md)。
...
| 优先级 | 事项 | 当前边界 |
|---|---|---|
| P0 | UnityFarm 0.12.3 消费验收 | 确认 UPM 解析 `v0.12.3`，校验字体 SHA256，手动编译并验收六个正式 SceneUI 中文标记；原配置表消费回归仍需完成 |
