...
> 当前发布版本为 **0.11.4**；消费项目安装与运行验收单独记录。
...
3. 添加框架 Git URL：`https://github.com/wsydet/ember-unity-framework.git?path=/Packages/com.ember#v0.11.4`；需要完整开发环境时同时按包名合并随包 `Dependencies~/manifest-0.11.4.json`，不覆盖消费项目其他依赖。
...
`source3d-2p5d 0.2.7 / preview`，共同声明框架 `0.11.4`；派生父基线已为 `base 0.5.6`。0.11.4 为 GameplayScene 和 2.5D Input Actions 更换 Ember 专用稳定 GUID，并在模板写入前检查不会被本次替换的项目资源是否已占用模板 GUID；消费项目只能部署包内模板，模板写 API 仅允许 embedded 框架项目。
...
当前升级器只更新 `com.ember`，Odin/DOTween 只检测存在与提供安装按钮。随包 `Dependencies~/release-0.11.4.json` 和完整 manifest 基线继续使用第三方 `ember-v0.11.1`；消费端差异确认、权限检查和自动安装流程仍未实现。消费端需手动合并声明，不能复制开发机 `file:` 路径或覆盖其整份 manifest。
...
包和模板兼容版本为 **0.11.4**；内容 hash、父基线和 GUID 引用已复核。第三方内容未变化，复用 `ember-v0.11.1`；框架发布信息与消费验收边界见 [0.11.4 发布说明](release-0.11.4.md)。
