...
> 当前用于 0.11.3 发布与消费端验收。模板 hash 和版本静态检查已完成；当前 Unity MCP 不可用，Unity 编译、EditMode、Unity Farm 安装、替换部署与 Play Mode 均按未执行处理。

本轮状态：包与父子模板已声明 0.11.3；模板内容未改，文本 hash 跨 CRLF/LF 稳定。项目中心只允许消费端部署目标完整模板，所有模板编辑写 API 限制为 embedded 框架项目；不能由静态检查推断 Unity Farm 的安装或运行兼容已经通过。
