...
> 当前用于 0.12.0 发布与消费端验收。Unity 编译、EditMode、模板保存/同步、新消费项目安装和 UnityFarm 升级均以本轮实际执行记录为准。

本轮状态：0.12.0 Table 实现与发布声明已收口；用户确认 Unity 零编译错误并报告三套 Table EditMode 通过。`base 0.6.0` 与 `source3d-2p5d 0.3.1` 已经项目中心保存、封存、父同步并声明兼容框架 `0.12.0`，实时/封存/父快照 Hash 一致；UnityFarm 更新和新消费项目完整回归作为发布后验收。本会话没有 Unity MCP，不把静态检查写成自动编译结论。
