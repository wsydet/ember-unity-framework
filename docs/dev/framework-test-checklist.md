...
> 当前用于 0.11.2 发布与消费端验收。模板 hash 和版本静态检查已完成；当前 Unity MCP 不可用，Unity 编译、EditMode、Unity Farm 安装、切换与 Play Mode 均按未执行处理。

本轮状态：包与父子模板已声明 0.11.2；模板内容未改，文本 hash 改为跨 CRLF/LF 稳定。项目中心已支持完整模板备份切换，消费端自动依赖同步仍未实现，不能由框架侧测试推断 Unity Farm 的安装或运行兼容已经通过。
