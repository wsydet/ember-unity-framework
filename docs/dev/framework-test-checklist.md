...
> 当前用于 0.11.1 发布准备。2026-09-07 已记录模板分支与场景语义同步专项通过；其余项目保持未执行，不从旧版本结果推断。

本轮准备状态：包与父子模板已声明 0.11.1，内容/父基线 hash 不变，未发布；派生模板 Assets 已从匹配 0.2.6 metadata 的备份恢复，仍需重新验证正常保存。Rainbow 两包、Console Pro、InputDeviceDetector 和 Feel 纳入私有仓交付；Feel 仅完成本地 UPM 封装，原 Assets 插件仍保留，待切换安装及 Unity 验收。随包 56 项依赖清单已准备，消费端自动同步未实现，不能据声明变更推断安装或运行兼容已通过。
