...
| [EmberDebugConfigSO.cs](../../Assets/Ember/Core/Runtime/Debug/EmberDebugConfigSO.cs) | ScriptableObject | `FoldoutGroup($const)` + `BoxGroup(ShowLabel=false)` + `GUIColor($prop)` + `InfoBox(VisibleIf)` + `ListDrawerSettings` + `HorizontalGroup/HideLabel` | 2026-08-01 |
...
| 脚本 | 类型 | Odin 模式 | 最后更新 |
|------|------|-----------|----------|
| [EmberDebugConfigEditor.cs](../../Assets/Ember/Core/Editor/Debug/EmberDebugConfigEditor.cs) | OdinEditor | `PropertyOrder` + `HorizontalGroup` + `Button` | — |
