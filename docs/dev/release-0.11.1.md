...
- 框架：`Packages/com.ember/package.json` 为 0.11.1。
- 模板：`base 0.5.5` 与 `source3d-2p5d 0.2.6` 均声明兼容框架 0.11.1；内容、版本、hash、父基线和当前编辑记录不变。声明前已逐树复核内容 hash。
- 消费端：`Packages/com.ember/Dependencies~/manifest-0.11.1.json` 包含当前工程 55 项直接依赖加 Feel，共 56 项；不含开发机 `file:` 路径，MCP 固定到当前 lock 的 commit。
- 第三方：`ember-thirdparty-upm/release-ember-0.11.1.json` 列出 7 个包及其版本，消费端统一引用新 tag `ember-v0.11.1`，旧插件 tag 保持不变。

当前尚未提交本次 0.11.1 声明，也未打 tag 或推送，并且未完成 Unity 编译、Feel 的 UPM 安装切换和消费端功能验证。声明不等于验收通过。错误的旧 `v0.11.0` / `ember-v0.11.0` 不移动、不覆盖。
...
4. 框架脚本将提交 `Packages/com.ember` 与 `docs` 下的 0.11.1 发布声明改动，不包含项目 `Assets`、`.agents`、`.claude` 和其他目录。必须先审阅暂存清单；若其中有不应发布的改动，停止并先拆分提交。
...
if (git tag --list 'ember-v0.11.1' 'feel-v5.4.0') { throw '目标 tag 已存在，请先核对，禁止覆盖。' }
Invoke-ReleaseGit diff --cached --quiet
Invoke-ReleaseGit add -- README.md release-ember-0.11.1.json
...
Invoke-ReleaseGit commit -m 'release: third-party bundle for Ember 0.11.1'
Invoke-ReleaseGit tag -a ember-v0.11.1 -m 'Third-party dependency baseline for Ember 0.11.1'
Invoke-ReleaseGit tag -a feel-v5.4.0 -m 'Feel 5.4.0 team UPM package'
Invoke-ReleaseGit push --atomic origin main refs/tags/ember-v0.11.1 refs/tags/feel-v5.4.0
}
```
...
if (git tag --list v0.11.1) { throw 'v0.11.1 已存在，请先核对，禁止覆盖。' }
Invoke-ReleaseGit diff --cached --quiet
Invoke-ReleaseGit ls-remote --exit-code --tags 'https://github.com/wsydet/ember-thirdparty-upm.git' refs/tags/ember-v0.11.1
...
Invoke-ReleaseGit commit -m 'release: Ember Framework 0.11.1'
Invoke-ReleaseGit tag -a v0.11.1 -m 'Ember Framework 0.11.1'
Invoke-ReleaseGit push --atomic origin main refs/tags/v0.11.1
}
```
...
```text
https://github.com/wsydet/ember-unity-framework.git?path=/Packages/com.ember#v0.11.1
```
