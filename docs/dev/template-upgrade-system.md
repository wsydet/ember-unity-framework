...
> 框架 package.json 与父子模板当前为 `0.11.5`。schema v2、父子同步、场景合并、部署前 GUID 冲突预检与消费端完整模板部署已实现；消费项目安装与运行验收单独记录。
...
二者已声明框架 `0.11.5`。0.11.5 未修改模板 Assets、内容版本、hash 或父基线，只推进兼容声明；0.11.4 的稳定 GUID 和部署保护继续有效，消费端验证仍需在实际项目执行。
