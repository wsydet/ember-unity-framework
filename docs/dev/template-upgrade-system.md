...
> 当前已发布框架与父子模板兼容声明均为 `0.12.0`。schema v2、父子同步、场景合并、部署前 GUID 冲突预检与消费端完整模板部署已实现；消费项目安装与运行验收单独记录。
...
已发布 0.12.0 模板为 `base 0.6.0 / stable` 与 `source3d-2p5d 0.3.1 / preview`，派生父基线为 `base 0.6.0`，内容 Hash、封存 Hash 与 ParentSnapshot 已校验一致；两份模板已按正式根声明和父同步流程声明兼容框架 `0.12.0`。消费端验证仍需在实际项目执行。
