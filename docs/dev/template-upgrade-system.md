...
> 当前已发布框架为 `0.11.5`；工作区 package.json 与 0.12.0 父子模板兼容声明均为 `0.12.0`。schema v2、父子同步、场景合并、部署前 GUID 冲突预检与消费端完整模板部署已实现；消费项目安装与运行验收单独记录。
...
已发布 0.11.5 基线为 `base 0.5.6 / stable` 与 `source3d-2p5d 0.2.7 / preview`。当前 0.12.0 候选已通过项目中心封存 `base 0.6.0` 与 `source3d-2p5d 0.3.1`，派生父基线为 `base 0.6.0`，内容 Hash、封存 Hash 与 ParentSnapshot 已校验一致；两份模板已按正式根声明和父同步流程把兼容框架推进到 `0.12.0`。消费端验证仍需在实际项目执行。
