...
> 2026-09-22 未发布扩展：框架级 Skill 随包首次附带；模板自有/继承 Skill 源文件纳入五目录快照，发现副本由原安装器暂存，与模板目录和身份记录一起提交。首次补缺现也先暂存再事务提交。消费端技能修复要求正式部署 version/hash 与包内封存版本一致，包升级不静默更新已有技能。详情与未完成的 Unity 验证见 [框架与模板 AI Skill](../../Packages/com.ember/Documentation~/maintenance/template-ai-skills.md)。以下既有历史验收不代表这批新代码已验证。
