...
从 [框架包说明](../Packages/com.ember/README.md) 了解安装与入口，从 [当前进度](dev/framework-progress.md) 了解已有能力和待办。当前发布版本为 **0.12.1**，在 0.12.0 强类型配置表基础上增加声明/数据可视化、可复制查询代码和安全单表导出。模板内容未变化，现有 0.12.0 兼容声明按 major.minor 规则继续适用。
