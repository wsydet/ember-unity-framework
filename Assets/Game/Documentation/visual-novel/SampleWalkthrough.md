...
> **0.10.1 修复批对本样例的影响（无内容改动）**：四个对话节点的段首都是
> `Background Show lastlight_rooftop` + 0.3 秒过渡，也就是官方推荐的「节点开头重新声明场景背景」。
> 以前同键重设会被当成换景、从全透明淡入，所以每次跨节点都会看到整屏一黑再亮回来；
> 现在同键重设是幂等的，背景保持不显示中断，这四个节点的观感因此变干净了，**时长仍是 0.3 秒**。
> 阅读页新增的不透明底板也顺带修掉了背景淡入头几帧与章节卡退场渐隐时可能透出页面后画面的问题。
> LastLight 的指令、台词、选项、分支和语义指纹都没有变化，旧存档不受影响。
