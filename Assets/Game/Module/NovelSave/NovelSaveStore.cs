...
namespace Game.NovelSave
{
    ...
    public sealed class NovelSaveStore
    {
        ...
        private NovelCheckpoint Decode(byte[] bytes)
        {
            ...
            if (checkpoint == null || (checkpoint.SchemaVersion < 1 || checkpoint.SchemaVersion > NovelCheckpoint.CurrentSchemaVersion) || string.IsNullOrWhiteSpace(checkpoint.StoryPath))
