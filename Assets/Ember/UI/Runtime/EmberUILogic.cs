...
namespace Ember.UI
{
    ...
    /// 每个 EmberPage 持有一个 EmberUILogic 实例，负责所有业务逻辑。
    /// MonoBehaviour 生命周期由 EmberPage 桥接到此类。
    ///
    /// <para>架构：</para>
    /// <code>
    /// EmberPage (MonoBehaviour on prefab)   ← 表现层（框架管理）
    ///   └── EmberUILogic (plain class)      ← 逻辑层（生成的代码 + 手写逻辑）
    ...
    /// <para>生成的代码示例：</para>
    /// <code>
    /// public partial class UIMainMenu : EmberUILogic
    /// {
    ...
    public class EmberUILogic
    {
        ...
        /// <summary>所属 EmberPage（MonoBehaviour）</summary>
        public EmberPage Page { get; set; }

        /// <summary>是否需要每帧 Update（默认 false）</summary>
        public bool NeedUpdate { get; set; }

        // 嵌套子 Logic（对标 Burner behaviours 列表，子 UIBinding 独立管理）
        private readonly List<EmberUILogic> _childLogics = new();
        ...
        // ═══════════════════════════════════════════════════════════
        // 注意：EmberUILogic 的所有钩子都是纯业务逻辑，不要在这里写动画。
        // 动画写在 EmberPage 子类的 OnShow() / OnHide() virtual 方法中，
        ...
        /// </summary>
        /// <para><b>触发时机：</b><see cref="EmberPage.CreateLogic"/> 中，ControlMap 填充完毕后立即调用。</para>
        ...
        /// </summary>
        /// <para><b>触发时机：</b><see cref="EmberPage.Init"/> 中，在 OnBind 之后、OnOpen 之前。
        /// 此时 ControlMap 已就绪、控件引用可用，页面尚未可见（α=0）。</para>
        /// <para><b>在这里做：</b>注册按钮事件（onClick.AddListener）、设置初始默认值。</para>
        /// <para><b>不要在这里做：</b>播放动画（那是 <see cref="EmberPage.OnShow()"/> 的事）、
        ...
        /// </summary>
        /// <para><b>触发时机：</b><see cref="EmberPage.Init"/> 中 OnInit 之后，
        /// 以及 <see cref="EmberPage.OnReopen"/> 时。</para>
        /// <para><b>在这里做：</b>根据传入参数切换页面状态（如从哪个入口进来的、携带的初始数据）。</para>
        /// <para><b>不要在这里做：</b>注册事件（那是 <see cref="OnInit"/> 的事）、
        /// 播放动画（那是 <see cref="EmberPage.OnShow()"/> 的事）。</para>
        /// <param name="param">打开时传入的参数（来自 EmberUIPageRouter.ShowMainPage/ShowPopup 的 args）</param>
        public virtual void OnOpen(object param) { }

        /// <summary>
        /// 页面即将可见（业务逻辑） —— 在 <b>PlayShow 阶段、打开动画之前</b>调用。
        /// </summary>
        /// <para><b>触发时机：</b><see cref="EmberPage.PlayShow"/> 中，在调用 <see cref="EmberPage.OnShow()"/>
        ...
        ///   <item><b>自定义方式</b>：创建 <see cref="EmberPage"/> 子类，override <see cref="EmberPage.OnShow()"/>，
        ///       返回 <c>IEnumerator</c> 协程（非 EmberUILogic 子类）。参见 <see cref="EmberPage.OnShow()"/> 的文档和示例</item>
        /// </list>
        /// </para>
        ...
        /// </summary>
        /// <para><b>触发时机：</b><see cref="EmberPage.PlayHide"/> 中，在调用 <see cref="EmberPage.OnHide()"/>
        /// （关闭动画）<b>之前</b>。</para>
        /// <para><b>在这里做：</b>停止实时刷新、停止音频、准备关闭。</para>
        /// <para><b>⚠️ 不要在这里写动画！</b>关闭动画写在：
        /// <list type="bullet">
        ///   <item><b>预设方式</b>：Inspector 中 UIBinding 勾选"启用预设渐入渐出"，设时间即可</item>
        ///   <item><b>自定义方式</b>：创建 <see cref="EmberPage"/> 子类，override <see cref="EmberPage.OnHide()"/>，
        ///       返回 <c>IEnumerator</c> 协程（非 EmberUILogic 子类）。参见 <see cref="EmberPage.OnHide()"/> 的文档和示例</item>
        /// </list>
        /// </para>
        ...
        /// </summary>
        /// <para><b>触发时机：</b><see cref="EmberPage.Cleanup"/> 中，在 OnReset 和 OnDispose 之前。
        ...
        /// </summary>
        /// <para><b>触发时机：</b><see cref="EmberPage.Cleanup"/> 末尾，在 OnClose 和 OnReset 之后。
        ...
        #region 广播方法（由 EmberPage 调用，内部先调 virtual hook 再递归子 Logic）
