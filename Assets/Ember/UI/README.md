...
基于层级栈的界面管理系统。每个界面实现 `IUIView` 接口获得完整生命周期，
通过 `PageDef` 注册页面元数据，由 `EmberUIManager` 管理多层级 Push/Pop。

## 文件清单

| 角色 | 路径 |
|------|------|
| 主逻辑入口 | `Runtime/EmberUIManager.cs` |
| 核心接口 | `Runtime/IUIView.cs` |
| 页面定义 | `Runtime/PageDef.cs` |
...
| 方法 | 说明 |
|------|------|
| `Push(PageDef page, object args)` | 加载预制体 → 实例化 → OnOpen → 入栈。暂停原栈顶 |
| `Pop(int layer)` | 关闭栈顶：OnClose → Destroy → 恢复新栈顶 |
| `CloseAll(int layer)` | 关闭指定层级所有界面 |
| `CloseAll()` | 关闭所有层级所有界面 |
| `GetTopView(int layer) → IUIView` | 获取栈顶界面 |
...
**Push：** `Push(page, args)` → 初始化检查 → EnsureLayerRoot → PauseTopView → ResourceManager.LoadAssetAsync → Instantiate → GetComponent<IUIView> → 无组件则 Destroy → stack.Push → OnOpen
...
| 预制体要求 | 预制体根节点必须有实现 IUIView 的 MonoBehaviour 组件 |
