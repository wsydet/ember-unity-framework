...
| # | 文件 | 状态 | 改动摘要 |
|---|------|------|----------|
| 1 | path/to/file.cs | 新增 | 新增了 XXX 方法 |
| 2 | path/to/file.cs.meta | 新增 | 配套 meta |
| 3 | path/to/other.cs | 修改 | 调整了 YYY 逻辑 |
| 4 | path/to/old.cs | 删除 | 已迁移至新位置 |
| 5 | path/to/old.cs.meta | 删除 | 配套 meta |

```bash
git add path/to/file.cs path/to/file.cs.meta \
  path/to/other.cs \
  path/to/old.cs path/to/old.cs.meta
